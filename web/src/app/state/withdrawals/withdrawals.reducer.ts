...
export const withdrawalReducer = createReducer(
    ...
    on(approveWithdrawal, (state) => ({ ...state, status: 'loading' as const })),
    on(approveWithdrawalSuccess, (state, { withdrawal }) => ({
        ...state,
        withdrawals: state.withdrawals.map(w => w.id === withdrawal.id ? withdrawal : w),
        status: 'success' as const
    })),
    on(approveWithdrawalFailure, (state, { error }) => ({ ...state, errorMessage: error, status: 'error' as const })),

    on(rejectWithdrawal, (state) => ({ ...state, status: 'loading' as const })),
    on(rejectWithdrawalSuccess, (state, { withdrawal }) => ({
        ...state,
        withdrawals: state.withdrawals.map(w => w.id === withdrawal.id ? withdrawal : w),
        status: 'success' as const
    })),
