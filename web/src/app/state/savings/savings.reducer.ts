...
export const savingReducer = createReducer(
    ...
    on(approveSaving, (state) => ({ ...state, status: 'loading' as const })),
    on(approveSavingSuccess, (state, { saving }) => ({
        ...state,
        savings: state.savings.map((s) => (s.id === saving.id ? saving : s)),
        status: 'success' as const
    })),
    on(approveSavingFailure, (state, { error }) => ({
        ...state,
        errorMessage: error,
        status: 'error' as const
    })),
    on(rejectSaving, (state) => ({ ...state, status: 'loading' as const })),
    on(rejectSavingSuccess, (state, { saving }) => ({
        ...state,
        savings: state.savings.map((s) => (s.id === saving.id ? saving : s)),
        status: 'success' as const
    })),
