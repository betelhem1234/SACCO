...
import {
    ...
    searchMembers, searchMembersSuccess, searchMembersFailure,
...
export class MembersEffects {
    ...
    searchMembers$ = createEffect(() =>
        this.actions$.pipe(
            ofType(searchMembers),
            switchMap(action =>
                this.apiService.searchMembers(
                    action.pageSize,
                    action.pageIndex,
                    action.sortField,
                    ...
                    map(result => searchMembersSuccess({ data: result.data, total: result.total })),
                    catchError(error => of(searchMembersFailure({ error })))
                )
            )
        )
    );
